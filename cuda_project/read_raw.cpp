...
void Meshio::calculate_grid(vector<vertex3D> &origin)
{
	origin.clear();
    origin.resize(ndivz*ndivy*ndivx);
    for (int k = 0; k < ndivz; k++)
	{
		for (int j = 0; j < ndivy; j++)
		{
			for (int i = 0; i < ndivx; i++)
			{
				...
				origin[e].x = x_min + ((x_max - x_min) / ((ndivx - 1)*i));
				origin[e].y = y_min + ((y_max - y_min) / ((ndivy - 1)*j));
                origin[e].z = z_min + ((z_max - z_min) / ((ndivz - 1)*k));
            }
        }
    }
}
